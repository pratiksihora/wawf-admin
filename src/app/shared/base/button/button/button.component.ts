...
import { Expression } from 'src/app/shared/constants/models/controls/form/form-field-config';
import { ConfirmConfig } from 'src/app/shared/constants/models/controls/modal/confirm-modal/config';
...
export class ButtonComponent implements OnInit {
  ...
  _checkPermissions() {
    ...
    if (this.btnOption?.permission) {
      ...
      const permission = permissions.some(x => this.permissionService?.getPermission(x));
      if (!permission) {
        this.hidden = true;
        return;
      }
  ...
  click() {
    if (this.btnOption?.confirmation) {
      this.confirmationService.show(ComfirmationUtil.configure(this.btnOption?.confirmation), (response: any) => {
