...
import { PrimeTemplate, SharedModule, FilterMatchMode, FilterOperator, SelectItem, PrimeNGConfig, TranslationKeys, FilterService, OverlayService } from '../api';
import { PaginatorModule } from '../paginator';
import { InputTextModule } from '../inputtext';
import { ButtonModule } from '../button';
import { SelectButtonModule } from '../selectbutton';
import { TriStateCheckboxModule } from '../tristatecheckbox';
// import { CalendarModule } from '../calendar';
import { InputNumberModule } from '../inputnumber';
import { DropdownModule } from '../dropdown';
import { DomHandler, ConnectedOverlayScrollHandler } from '../dom';
import { ObjectUtils, UniqueComponentId, ZIndexUtils } from '../utils';
import { SortMeta } from '../api';
import { TableState } from '../api';
import { FilterMetadata } from '../api';
...
@Component({
    ...
    template: `
        <div #container [ngStyle]="style" [class]="styleClass"
            ...
            <div class="d-flex flex-column flex-fill w-100" [ngClass]="{'d-none' : layoutType === 'grid'}" [hidden]="layoutType === 'grid'"> 
                ...
                <div #wrapper class="p-datatable-wrapper flex-fill overflow-x-auto" [ngStyle]="{height: scrollHeight}">
            ...
            <div class="d-flex flex-column flex-fill w-100" [ngClass]="{'d-none' : layoutType !== 'grid'}" [hidden]="layoutType !== 'grid'">
                <ng-container *ngIf="gridTemplate">
                    <ng-container
                        *ngTemplateOutlet="gridTemplate; context: { columns: columns,  value: filteredValue|| value, selection: selection  }">
                    </ng-container>
                </ng-container>
                <p-paginator [rows]="rows" [first]="first" [totalRecords]="totalRecords" [pageLinkSize]="pageLinks" styleClass="p-paginator-bottom" [alwaysShow]="alwaysShowPaginator"
                    (onPageChange)="onPageChange($event)" [rowsPerPageOptions]="rowsPerPageOptions" *ngIf="paginator && (paginatorPosition === 'bottom' || paginatorPosition =='both')"
...
export class Table implements OnInit, AfterViewInit, AfterContentInit, BlockableUI, OnChanges {
    ...
    filter(value: any, field: string, matchMode: string, config: any = null) {
        if (this.filterTimeout) {
            clearTimeout(this.filterTimeout);
        }
        if (!this.isFilterBlank(value)) {
            this.filters[field] = { value: value, matchMode: matchMode, config };
...
@Component({
    selector: 'p-sortIcon',
    template: `
        <i class="p-sortable-column-icon bi" [ngClass]="{'bi-arrow-up': sortOrder === 1, 'bi-arrow-down': sortOrder === -1, 'bi-arrow-down-up': sortOrder === 0}"></i>
    ...
    template: `
        <div class="p-radiobutton p-component" [ngClass]="{'p-radiobutton-focused':focused, 'p-radiobutton-disabled': disabled}" (click)="onClick($event)">
            ...
            <div class="form-check form-check-custom form-check-solid">
                <input class="form-check-input" type="radio" [attr.id]="inputId" [attr.name]="name" [checked]="checked" (focus)="onFocus()" (blur)="onBlur()"
                [disabled]="disabled" [attr.aria-label]="ariaLabel"/>
    ...
    template: `
        <div class="p-checkbox p-component" [ngClass]="{'p-checkbox-focused':focused, 'p-checkbox-disabled': disabled}" (click)="onClick($event)">
            ...
            <div #box [ngClass]="{'p-checkbox-box p-component':true,
                ...
                <span class="p-checkbox-icon" [ngClass]="{'bi bi-check':checked}"></span>
            </div> -->
            <div class="form-check form-check-custom form-check-solid cursor-pointer">
                <input class="form-check-input cursor-pointer" type="checkbox" [attr.id]="inputId" [attr.name]="name" [checked]="checked" (focus)="onFocus()" (blur)="onBlur()" [disabled]="disabled"
                [attr.required]="required" [attr.aria-label]="ariaLabel"/>
                <label class="form-check-label"></label>
            </div>
        </div>
...
@Component({
    ...
    template: `
        <div class="p-checkbox p-component" [ngClass]="{'p-checkbox-focused':focused, 'p-checkbox-disabled': isDisabled()}" (click)="onClick($event)">
            ...
            <div class="form-check form-check-custom cursor-pointer">
                <input class="form-check-input bg-gray-300 cursor-pointer" #cb type="checkbox" [attr.id]="inputId" [attr.name]="name" [checked]="checked" (focus)="onFocus()" (blur)="onBlur()"
                [disabled]="isDisabled()" [attr.aria-label]="ariaLabel"/>
                <label class="form-check-label"></label>
            </div>
        </div>
...
@NgModule({
    imports: [CommonModule, PaginatorModule, InputTextModule, DropdownModule, ScrollingModule, FormsModule, ButtonModule, SelectButtonModule, InputNumberModule, TriStateCheckboxModule],
