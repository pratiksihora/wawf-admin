...
import {Message, PrimeNGConfig} from '../api';
import {DomHandler} from '../dom';
import {PrimeTemplate,SharedModule} from '../api';
import {MessageService} from '../api';
import {ObjectUtils, UniqueComponentId} from '../utils';
        ...
        <div #container [attr.id]="message.id" [class]="message.styleClass" [ngClass]="['p-toast-message-' + message.severity, 'p-toast-message']" [@messageState]="{value: 'visible', params: {showTransformParams: showTransformOptions, hideTransformParams: hideTransformOptions, showTransitionParams: showTransitionOptions, hideTransitionParams: hideTransitionOptions}}"
            ...
            <div class="p-toast-message-content" role="alert" aria-live="assertive" aria-atomic="true"  [ngClass]="message.contentStyleClass">
                <ng-container *ngIf="!template">
                    <span [class]="'p-toast-message-icon bi pi' + (message.icon ? ' ' + message.icon : '')" [ngClass]="{'pi-info-circle': message.severity == 'info', 'pi-exclamation-triangle': message.severity == 'warn',
                        'pi-times-circle': message.severity == 'error', 'bi-check' :message.severity == 'success'}"></span>
                ...
                <button type="button" class="p-toast-icon-close p-link flex-shrink-0 text-dark h-25px w-25px d-flex align-items-center justify-content-center" (click)="onCloseIconClick($event)" (keydown.enter)="onCloseIconClick($event)" *ngIf="message.closable !== false" pRipple>
                    <i class="bi bi-x fs-2 fw-semibold d-flex justify-content-center align-items-center"></i>
                </button>
            </div>
        </div>
    `,
